...
function SecretSection({
  ...
  // Modal rendered via portal to document.body so it escapes
  // all stacking contexts (backdrop-filter on GlassCards, etc.)
  const modal =
    state === "unlocked"
      ? createPortal(
          <div
          ...
          >
            ...
            >
              ...
              <p className="text-white/90 text-sm leading-relaxed mb-6">
                You unlocked the most private part of my heart. ❤️ I really,
                really like you. You're special.
              </p>
    ...
    <div className="relative">
      ...
      {state === "locked" && (
        <GlassCard className="p-8 text-center">
          ...
          <p className="text-rose-600/70 mb-6">Only for the one who knows...</p>
          <button
            type="button"
        ...
        <GlassCard className="p-8 text-center">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button
              ...
              className="px-6 py-2 rounded-full font-bold text-rose-600 border border-rose-400/40 hover:bg-rose-50 transition-colors"
            >
              Cancel
            </button>
          </div>
      ...
      {/* Denied state */}
      {state === "denied" && (
        <GlassCard className="p-8 text-center animate-shake">
          <div className="text-5xl mb-4">🚫</div>
          <h3 className="text-xl font-bold text-red-600">Wrong key 🚫</h3>
          <p className="text-rose-600/70 text-sm mt-2">
            That's not quite right...
          </p>
        </GlassCard>
      )}

      {/* Portal modal — rendered directly on document.body */}
      {modal}
    </div>
  );
}
...
export default function RomiiPage() {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(160deg, #fff0f3, #fdf2ff, #fff0f8, #fff0f3)",
    ...
    >
      ...
      {/* Back button — always visible, goes behind portal modal naturally */}
      <button
        type="button"
        data-ocid="romii.link"
        onClick={() => navigate({ to: "/" })}
        className="fixed top-4 left-4 glass px-4 py-2 rounded-full text-sm font-semibold text-rose-700 hover:scale-105 transition-transform z-40"
      >
        ← Home
      </button>
      ...
      <section className="relative flex flex-col items-center justify-center pt-24 pb-16 px-4 text-center overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          <PlaceholderImage
            ...
            glow="0 0 60px 20px rgba(251,113,133,0.35), 0 0 120px 40px rgba(124,58,237,0.2)"
          />
          <h1
            ...
            style={{
              ...
              filter: "drop-shadow(0 0 15px rgba(190,24,93,0.3))",
            }}
          >
            Romii 🌹
          </h1>
          <p className="text-lg md:text-xl font-semibold text-rose-700/80 max-w-md">
          ...
          <div className="flex flex-col gap-6 pl-16">
            {storyEvents.map((ev, i) => (
              <ScrollFadeIn key={ev.title} delay={i * 120}>
                <GlassCard className="p-5 shimmer-card">
                  <div
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-rose-600/70 mt-1">{ev.desc}</p>
                    </div>
                  </div>
                </GlassCard>
              </ScrollFadeIn>
            ...
            <ScrollFadeIn key={card.title} delay={i * 60}>
              <GlassCard className="p-5 shimmer-card hover:scale-105 transition-transform">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-rose-700">{card.title}</h3>
                    <p className="text-sm text-rose-600/70 mt-1">{card.desc}</p>
                  </div>
                </div>
              </GlassCard>
            </ScrollFadeIn>
      ...
      <section className="max-w-4xl mx-auto px-4 pb-16 relative">
        <ScrollFadeIn>
          ...
          <p className="text-center text-rose-600/60 text-sm mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {abilities.map((ab, i) => (
            <ScrollFadeIn key={ab.title} delay={i * 100}>
              <GlassCard className="p-6 text-center shimmer-card">
                <div
                  className="inline-flex items-center justify-center w-14 h-14 rounded-full mb-3 text-2xl"
                  style={{
                    background:
                      "linear-gradient(135deg, rgba(251,113,133,0.2), rgba(124,58,237,0.2))",
                  }}
                >
                  {ab.emoji}
                </div>
                <div
                  ...
                  style={{ color: "rgba(190,24,93,0.7)" }}
                >
                  Ability
                </div>
                <h3 className="font-bold text-rose-700 mb-2">{ab.title}</h3>
                <p className="text-sm text-rose-600/70">{ab.desc}</p>
              </GlassCard>
            </ScrollFadeIn>
          ))}
        </div>
      ...
      <section className="max-w-4xl mx-auto px-4 pb-16 relative overflow-hidden">
        ...
        <ScrollFadeIn>
          <h2 className="text-3xl font-bold text-center text-rose-700 mb-8 relative z-10">
            Our Moments 🌙
          </h2>
        </ScrollFadeIn>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
          {moments.map((m, i) => (
            <ScrollFadeIn key={m.title} delay={i * 100}>
              <GlassCard className="p-6 shimmer-card hover:scale-105 transition-transform">
                <div className="text-3xl mb-2">{m.emoji}</div>
                <h3 className="font-bold text-rose-700 mb-1">{m.title}</h3>
                <p className="text-sm text-rose-600/70">{m.desc}</p>
              </GlassCard>
            </ScrollFadeIn>
          ))}
        </div>
      ...
      <section className="max-w-lg mx-auto px-4 pb-20">
        <ScrollFadeIn>
          <h2 className="text-2xl font-bold text-center text-rose-600/60 mb-6">
            🔮 Private...
          </h2>
        </ScrollFadeIn>
      ...
      <footer className="text-center pb-8 px-4">
        <p className="text-rose-700/70 font-semibold mb-1">
